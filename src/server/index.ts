import { tool, type Config, type Hooks, type Plugin } from "@opencode-ai/plugin";
...
export function createServerHooks({ db, client }: { db: Database; client: any }): Hooks {
    return {
        config: async (cfg) => {
        ...
        "chat.message": async (_input, output) => {
            ...
            if (output.message.model?.providerID && output.message.model?.modelID) return;
