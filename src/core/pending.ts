...
function validatePendingSource(row: PendingConnectionRow): PendingConnection["source"] {
...
function validatePromptStatus(row: PendingConnectionRow): PendingConnection["promptStatus"] {
    if (!promptStatuses.includes(row.prompt_status as PendingConnection["promptStatus"])) {
        throw new Error(
...
function pendingConnectionFromRow(row: PendingConnectionRow): PendingConnection {
...
function pendingID(providerID: string, auth: AuthInfo, source: PendingConnection["source"]) {
...
export function createPendingConnection(
    ...
    const identity = { providerID, auth, source } satisfies PendingIdentity;
...
export function completePendingConnection(db: Database, id: string, alias: string) {
