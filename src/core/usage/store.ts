...
export function saveUsageSnapshot(db: Database, snapshot: ProviderUsageSnapshot) {
    const { rawRedacted, ...normalizedSnapshot } = snapshot;
    const rawRedactedJSON = Object.hasOwn(snapshot, "rawRedacted") ? JSON.stringify(rawRedacted) : null;
    ...
    if (snapshot.confidence === "exact" && snapshot.usedPercent !== undefined && snapshot.usedPercent < 100) {
...
export function getUsageSnapshot(db: Database, providerID: string, alias: string) {
    const row = db
        .query<{ normalized_json: string; raw_redacted_json: string | null }, [string, string]>(
    ...
    if (row.raw_redacted_json !== null) snapshot.rawRedacted = JSON.parse(row.raw_redacted_json);
    return snapshot;
}
