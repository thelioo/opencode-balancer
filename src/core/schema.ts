...
function inferAuthType(authJSON: string) {
    try {
        ...
        if (typeof parsed.type === "string" && authTypes.has(parsed.type)) return parsed.type;
    } catch {
...
function selectExisting(db: Database, table: string, columns: string[]) {
    ...
    return db.query<Record<string, unknown>, []>(`SELECT ${selected.join(", ")} FROM ${table}`).all();
}
...
function recreateAccounts(db: Database) {
    ...
    const insert = db.query<unknown, [string, string, string, string, number, number, number | null, number | null, number, number]>(
    ...
    for (const row of rows) {
        if (typeof row.provider_id !== "string" || typeof row.alias !== "string" || typeof row.auth_json !== "string") continue;
        ...
        insert.run(
            ...
            typeof row.rate_limited_until === "number" ? row.rate_limited_until : null,
...
function recreatePendingConnections(db: Database) {
    ...
    const insert = db.query<unknown, [string, string, string, string, string, number, string]>(
        `INSERT OR REPLACE INTO pending_connections_new (
             id, provider_id, auth_json, auth_type, source, captured_at, prompt_status
         ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
    );
    for (const row of rows) {
        if (typeof row.id !== "string" || typeof row.provider_id !== "string" || typeof row.auth_json !== "string") continue;
        const authType = typeof row.auth_type === "string" && authTypes.has(row.auth_type) ? row.auth_type : inferAuthType(row.auth_json);
        insert.run(
            row.id,
            row.provider_id,
...
function recreateUsageSnapshots(db: Database) {
    ...
    const insert = db.query<unknown, [string, string, number, string, string, string | null, string | null]>(
    ...
    for (const row of rows) {
        if (typeof row.provider_id !== "string" || typeof row.alias !== "string") continue;
        insert.run(
            row.provider_id,
            row.alias,
...
function recreateEvents(db: Database) {
    ...
    const insert = db.query<unknown, [string, string, string | null, string | null, string, number, string]>(
        `INSERT OR REPLACE INTO events_new (id, type, provider_id, alias, message, created_at, metadata_json)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
    );
    for (const row of rows) {
        if (typeof row.id !== "string" || typeof row.type !== "string" || typeof row.message !== "string") continue;
        insert.run(
            row.id,
            row.type,
...
function recreateProviderPriority(db: Database) {
    ...
    const insert = db.query<unknown, [string, number, string | null, number, number]>(
