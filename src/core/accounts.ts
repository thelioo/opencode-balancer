...
export function saveAccount(db: Database, providerID: string, aliasInput: string, auth: AuthInfo): Account {
    const alias = normalizeAlias(aliasInput);
    if (!alias) throw new Error("Invalid alias");

    const save = db.transaction(() => {
        const timestamp = now();
        db.query<unknown, [string, string, string, string, number, number, number, number]>(
            `INSERT INTO accounts (
             ...
             ON CONFLICT(provider_id, alias) DO UPDATE SET
                 ...
                 updated_at = excluded.updated_at`,
        ).run(providerID, alias, JSON.stringify(auth), auth.type, timestamp, timestamp, 0, 0);
...
export function updateAccountAuth(db: Database, providerID: string, aliasInput: string, auth: AuthInfo): Account | undefined {
    const alias = normalizeAlias(aliasInput);
    if (!alias) throw new Error("Invalid alias");
...
export function renameAccount(db: Database, providerID: string, fromAliasInput: string, toAliasInput: string): Account {
    ...
    if (fromAlias === toAlias) {
        ...
        if (!account) throw new Error(`Account not found: ${providerID}/${fromAlias}`);
        return account;
    }

    const rename = db.transaction(() => {
        const existing = getAccount(db, providerID, fromAlias);
        if (!existing) throw new Error(`Account not found: ${providerID}/${fromAlias}`);
        if (getAccount(db, providerID, toAlias)) throw new Error(`Account already exists: ${providerID}/${toAlias}`);
        ...
        const active = db
            .query<{ active_alias: string | null }, [string]>("SELECT active_alias FROM provider_state WHERE provider_id = ?")
            .get(providerID);
        if (active?.active_alias === fromAlias) setActiveAlias(db, providerID, toAlias);

        const account = getAccount(db, providerID, toAlias);
        if (!account) throw new Error(`Account not found: ${providerID}/${toAlias}`);
        return account;
    });
...
export function getAccount(db: Database, providerID: string, alias: string): Account | undefined {
...
export function listAccounts(db: Database, providerID?: string): Account[] {
    ...
    const rows = providerID
        ? db
              .query<AccountRow, [string]>(`${sql} WHERE provider_id = ? ORDER BY alias`)
              .all(providerID)
        : db.query<AccountRow, []>(`${sql} ORDER BY provider_id, alias`).all();
    return rows.map(accountFromRow);
}

export function setActiveAccount(db: Database, providerID: string, alias: string): Account {
    const normalizedAlias = normalizeAlias(alias);
    const account = getAccount(db, providerID, normalizedAlias);
    if (!account) throw new Error(`Account not found: ${providerID}/${normalizedAlias}`);
...
export function removeAccount(db: Database, providerID: string, alias: string) {
    ...
    const remove = db.transaction(() => {
        ...
        db.query<unknown, [string, string]>("DELETE FROM accounts WHERE provider_id = ? AND alias = ?").run(
            providerID,
            normalizedAlias,
        );
        db.query<unknown, [string, string]>("DELETE FROM usage_snapshots WHERE provider_id = ? AND alias = ?").run(
            providerID,
            normalizedAlias,
        );
        ...
        if (active?.active_alias === normalizedAlias) {
            const next = db
                .query<{ alias: string }, [string]>("SELECT alias FROM accounts WHERE provider_id = ? ORDER BY alias LIMIT 1")
                .get(providerID);
...
export function getActiveAccount(db: Database, providerID: string): Account | undefined {
    const row = db
...
export function getSelectedAccount(db: Database): Account | undefined {
    const selectedProvider = db
        .query<{ value: string }, []>("SELECT value FROM settings WHERE key = 'selected_provider_id'")
        .get();
...
export function setSelectedModel(db: Database, providerID: string, modelID: string): SelectedModel {
    const row = db
        .query<{ metadata_json: string | null }, [string]>("SELECT metadata_json FROM provider_state WHERE provider_id = ?")
        .get(providerID);
...
export function getSelectedModel(db: Database, providerID: string): SelectedModel | undefined {
    const row = db
        .query<{ metadata_json: string | null }, [string]>("SELECT metadata_json FROM provider_state WHERE provider_id = ?")
        .get(providerID);
