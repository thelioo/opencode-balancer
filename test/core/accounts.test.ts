...
function createDb() {
	const dbPath = join(
		mkdtempSync(join(tmpdir(), "opencode-balancer-accounts-")),
		"balancer.sqlite",
	);
	dbPaths.push(dbPath);
...
describe("getSelectedAccount", () => {
	test("returns the most recently activated account across all providers", async () => {
		...
		expect(getSelectedAccount(db)).toMatchObject({
			alias: "op1",
			providerID: "openai",
		});
	});
	...
	test("returns the last activation even when two activations happen in the same millisecond", () => {
		...
		saveAccount(db, "openai", "op1", { key: "op", type: "api" });
...
describe("renameAccount", () => {
	test("renames an account and preserves active selection", () => {
		const db = createDb();
		const auth = { key: "sk-openai-test", type: "api" } as const;
