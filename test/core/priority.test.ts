...
function createDb() {
    const dbPath = join(mkdtempSync(join(tmpdir(), "opencode-balancer-priority-")), "balancer.sqlite");
    dbPaths.push(dbPath);
...
describe("provider priority", () => {
    test("auto-includes every provider that has an account, enabled, alphabetical", () => {
        const db = createDb();
        saveAccount(db, "openai", "op1", { type: "api", key: "k" });
        saveAccount(db, "github-copilot", "gh1", { type: "api", key: "k" });
    ...
    test("excludes providers that no longer have accounts", () => {
        ...
        expect(listProviderPriority(db).map((e) => e.providerID)).toEqual(["openai"]);
    });
    ...
    test("moveProvider reorders priority and clamps at the ends", () => {
        ...
        saveAccount(db, "openai", "op1", { type: "api", key: "k" });
        saveAccount(db, "github-copilot", "gh1", { type: "api", key: "k" });
        saveAccount(db, "anthropic", "an1", { type: "api", key: "k" });
        // initial alpha: anthropic, github-copilot, openai

        moveProvider(db, "openai", -1); // openai up one -> anthropic, openai, github-copilot
        expect(listProviderPriority(db).map((e) => e.providerID)).toEqual(["anthropic", "openai", "github-copilot"]);
...
describe("resolveActiveSelection", () => {
    ...
    test("falls over to the next provider when all accounts are rate limited", () => {
        ...
        db.query("UPDATE accounts SET rate_limited_until = ? WHERE provider_id = 'github-copilot'").run(5000);
    ...
    test("prefers the active account within the chosen provider", () => {
        const db = createDb();
        saveAccount(db, "openai", "op1", { type: "api", key: "k1" });
        saveAccount(db, "openai", "op2", { type: "api", key: "k2" });
        setProviderModel(db, "openai", "gpt-5.5");
    ...
    test("tries another healthy account in the current provider before falling over cross-provider", () => {
        ...
        saveAccount(db, "openai", "full", { type: "api", key: "full" });
        saveAccount(db, "openai", "available", { type: "api", key: "available" });
        setProviderModel(db, "github-copilot", "claude-haiku-4.5");
        setProviderModel(db, "openai", "gpt-5.5");
        setActiveAccount(db, "openai", "full");
        db.query("UPDATE accounts SET rate_limited_until = ? WHERE provider_id = 'openai' AND alias = 'full'").run(5000);
