...
import { closeBalancerDatabase, openBalancerDatabase, secureDatabaseFiles } from "../../src/core/database";
...
describe("schema migrations", () => {
    ...
    test("records migration version one", () => {
        const db = openBalancerDatabase(tempDb());
        migrate(db);
        const row = db.query<{ version: number }, []>("SELECT version FROM schema_migrations").get();
    ...
    test("runs migration idempotently", () => {
        ...
        const row = db
            .query<{ count: number }, []>("SELECT COUNT(*) AS count FROM schema_migrations WHERE version = 1")
            .get();
    ...
    test("repairs incomplete existing version one databases", () => {
        ...
        const accountColumns = db.query<{ name: string }, []>("PRAGMA table_info(accounts)").all().map((row) => row.name);
        const pendingColumns = db
            .query<{ name: string }, []>("PRAGMA table_info(pending_connections)")
            .all()
            .map((row) => row.name);
        const account = db.query<{ auth_type: string }, []>("SELECT auth_type FROM accounts").get();
        const pending = db.query<{ auth_type: string }, []>("SELECT auth_type FROM pending_connections").get();
        const tables = db
            .query<{ name: string }, []>("SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name")
            .all()
            .map((row) => row.name);
        ...
        expect(db.query<{ name: string }, []>("PRAGMA table_info(provider_state)").all().map((row) => row.name)).toContain(
            "metadata_json",
        );
        ...
        expect(db.query<{ disabled: number; failures: number }, []>("SELECT disabled, failures FROM accounts").get()).toEqual({
            disabled: 0,
            failures: 0,
        });
    });
    ...
    test("opens a new usable database after closing the cached handle", () => {
        ...
        const row = reopened.query<{ version: number }, []>("SELECT version FROM schema_migrations").get();
        expect(row?.version).toBe(1);
    });
