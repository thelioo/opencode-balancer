...
import { getAccount, getActiveAccount, listAccounts, removeAccount, saveAccount, setActiveAccount } from "../../src/core/accounts";
import {
    claimPendingPrompt,
...
describe("accounts and pending connections", () => {
    test("saves a pending connection as an active account", () => {
        const database = db();
        const pending = createPendingConnection(database, "anthropic", { type: "api", key: "sk-ant-test" }, "http");
        const account = completePendingConnection(database, pending.id, "Work Account");
    ...
    test("deduplicates equivalent pending connections from concurrent watchers", () => {
        ...
        const second = createPendingConnection(database, "openai", auth, "auth-file");
    ...
    test("deduplicates oauth pending connections when only access token fields changed", () => {
        ...
        const first = createPendingConnection(
            database,
            "openai",
            { type: "oauth", refresh: "r", access: "a1", expires: 123 },
            "auth-file",
        );
        const second = createPendingConnection(
            database,
            "openai",
            { type: "oauth", refresh: "r", access: "a2", expires: 456 },
            "auth-file",
        );
    ...
    test("deduplicates oauth pending connections by token account id when refresh token changes", () => {
        ...
        const first = createPendingConnection(
            database,
            "openai",
            { type: "oauth", refresh: "r1", access: jwtWithChatGPTAccountID("acct-1"), expires: 123 },
            "auth-file",
        );
        const second = createPendingConnection(
            database,
            "openai",
            { type: "oauth", refresh: "r2", access: jwtWithChatGPTAccountID("acct-1"), expires: 456 },
            "auth-file",
        );
    ...
    test("claimPendingPrompt marks equivalent pending rows so another instance cannot prompt again", () => {
        ...
        expect(listPendingConnections(database).map((pending) => pending.promptStatus)).toEqual(["prompted", "prompted"]);
    });

    test("claimPendingPrompt marks oauth duplicate rows with refreshed access tokens", () => {
        const database = db();
        const first = createPendingConnection(database, "openai", { type: "oauth", refresh: "r", access: "a1", expires: 123 }, "auth-file");
        database
            .query(
                `INSERT INTO pending_connections (
                     id, provider_id, auth_json, auth_type, source, captured_at, prompt_status
            ...
            .run(
                "legacy-refreshed",
                "openai",
                JSON.stringify({ type: "oauth", refresh: "r", access: "a2", expires: 456 }),
                "oauth",
                "auth-file",
                Date.now(),
                "new",
    ...
    test("dismisses a pending connection so native auth does not recreate it", () => {
        const database = db();
        const auth = { type: "api", key: "sk-ant-test" } as const;
        const pending = createPendingConnection(database, "anthropic", auth, "auth-file");

        expect(removePendingConnection(database, pending.id)).toBe(true);

        expect(listPendingConnections(database)).toMatchObject([{ id: pending.id, promptStatus: "dismissed" }]);
        expect(createPendingConnection(database, "anthropic", auth, "auth-file")).toMatchObject({
            id: pending.id,
    ...
    test("rolls back account save when active state write fails", () => {
        ...
        expect(() => saveAccount(database, "openai", "main", { type: "api", key: "sk-main" })).toThrow(
    ...
    test("rejects account rows with mismatched auth types", () => {
        ...
        database
            ...
            .run("openai", "main", JSON.stringify({ type: "api", key: "sk-main" }), "oauth", Date.now(), Date.now());
    ...
    test("rejects pending rows with invalid sources", () => {
        ...
        database
            ...
            .run(
                ...
                "openai",
                JSON.stringify({ type: "api", key: "sk-main" }),
                "api",
                "invalid-source",
                Date.now(),
                "new",
