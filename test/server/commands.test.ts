...
describe("runFallbackBalancerCommand", () => {
	...
	test("status reports account and pending counts", () => {
		...
		expect(runFallbackBalancerCommand(database, "status")).toBe(
			"accounts=1 pending=1",
		);
	});

	test("active reports when a provider has no active account", () => {
		expect(runFallbackBalancerCommand(db(), "active openai")).toBe(
			"No active account for provider.",
		);
	});
	...
	test("use switches the active account", () => {
		const database = db();
		saveAccount(database, "openai", "main", { key: "sk-main", type: "api" });
		saveAccount(database, "openai", "work", { key: "sk-work", type: "api" });
