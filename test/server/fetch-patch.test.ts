...
import { __testResetFetchPatch, installFetchPatch } from "../../src/server/fetch-patch";
import {
...
describe("server fetch patch", () => {
    test("keeps a fresh native oauth authorization header instead of restoring a stale saved access token", async () => {
        ...
        setPendingRequest("request-1", { providerID: "openai", account });
    ...
    test("overrides oauth authorization on failover attempts so the retry uses the next account", async () => {
        ...
        setPendingRequest("request-1", { providerID: "openai", account: main });

        const attemptedAuthorizations: string[] = [];
        globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
            const headers = new Headers(init?.headers);
            attemptedAuthorizations.push(headers.get("authorization") ?? "");
            return new Response(attemptedAuthorizations.length === 1 ? "rate limited" : "ok", {
        ...
        expect(response.status).toBe(200);
        expect(attemptedAuthorizations).toEqual(["Bearer main-native-access", "Bearer backup-access"]);
    });

    test("bounds failover retries and marks each retryable attempt", async () => {
        const database = db();
        setBalancingEnabled(database, true);
        const main = saveAccount(database, "openai", "main", { type: "api", key: "sk-main" });
        saveAccount(database, "openai", "backup-1", { type: "api", key: "sk-backup-1" });
        saveAccount(database, "openai", "backup-2", { type: "api", key: "sk-backup-2" });
        saveAccount(database, "openai", "backup-3", { type: "api", key: "sk-backup-3" });
        setPendingRequest("request-1", { providerID: "openai", account: main });

        const attemptedKeys: string[] = [];
        globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
            const headers = new Headers(init?.headers);
            attemptedKeys.push(headers.get("authorization")?.replace(/^Bearer /, "") ?? "");
