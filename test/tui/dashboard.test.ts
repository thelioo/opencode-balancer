...
describe("Dashboard", () => {
    test("renders new account as an accounts row instead of a header action", () => {
        const source = readFileSync(join(import.meta.dir, "../../src/tui/components/dashboard.tsx"), "utf8");

        expect(source).toContain('type: "connect"');
        expect(source).toContain("New account");
        expect(source).toContain('<SectionTitle label="ACCOUNTS" count={accounts().length} />');
    ...
    test("mutes account usage bars unless their account row is selected", () => {
        ...
        expect(source).toContain("muted={!selected(`account:${account.providerID}/${account.alias}`)}");
    });
