...
import {
	...
	moveDashboardFocus,
	reduceDashboardKey,
} from "../../src/tui/dashboard-keys";

describe("reduceDashboardKey", () => {
	test("arrow and vim keys move the cursor", () => {
		expect(reduceDashboardKey({ name: "up" })).toEqual({
			delta: -1,
			type: "move-cursor",
		});
		expect(reduceDashboardKey({ name: "down" })).toEqual({
			delta: 1,
			type: "move-cursor",
		});
		expect(reduceDashboardKey({ name: "k" })).toEqual({
			delta: -1,
			type: "move-cursor",
		});
		expect(reduceDashboardKey({ name: "j" })).toEqual({
			delta: 1,
			type: "move-cursor",
		});
	});
	...
	test("left and right arrows move across header actions", () => {
		expect(reduceDashboardKey({ name: "left" })).toEqual({
			delta: -1,
			type: "move-header",
		});
		expect(reduceDashboardKey({ name: "right" })).toEqual({
			delta: 1,
			type: "move-header",
		});
		expect(reduceDashboardKey({ name: "h" })).toEqual({
			delta: -1,
			type: "move-header",
		});
		expect(reduceDashboardKey({ name: "l" })).toEqual({
			delta: 1,
			type: "move-header",
		});
	});
...
describe("moveDashboardFocus", () => {
	test("moves up from the first content row to the header", () => {
		expect(
			moveDashboardFocus({ area: "content", cursor: 0, rowCount: 3 }, -1),
		).toEqual({
			area: "header",
			cursor: 0,
		});
	...
	test("moves down from the header to the content rows", () => {
		expect(
			moveDashboardFocus({ area: "header", cursor: 2, rowCount: 3 }, 1),
		).toEqual({
			area: "content",
			cursor: 2,
		});
	...
	test("moves within content rows without leaving bounds", () => {
		expect(
			moveDashboardFocus({ area: "content", cursor: 1, rowCount: 3 }, -1),
		).toEqual({
			area: "content",
			cursor: 0,
		});
		expect(
			moveDashboardFocus({ area: "content", cursor: 2, rowCount: 3 }, 1),
		).toEqual({
			area: "content",
			cursor: 2,
		});
